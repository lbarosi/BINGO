---
title: "Working with Spatial Data in R: An Example"
...
erbs1 <- ERBs %>% as.data.frame()
ggplot()+
  geom_path(data = municipios, aes(x = long, y = lat, group = group))+
  geom_point(data = erbs1, aes(x=Longitude, y=Latitude), col = "red", pch="+", cex=1)+
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
sitiosZoom(61,0.2,50)
```

### Barra de Santa Rosa

```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
sitiosZoom(90,0.2,50)
```
...
dataSitios <- sites %>% as.data.frame() %>%   verificaSitios(.) %>% arrange(., -Ruído)
coordinates(dataSitios) <- c("lon","lat")
proj4string(dataSitios) <- proj4string(municipios)
dataSitios$AEREO1       <- dist2Line(dataSitios,l1) %>% as.data.frame() %>% select(., distance)
dataSitios$AEREO2         <- dist2Line(dataSitios,l2) %>% as.data.frame() %>% select(., distance)
dataSitios$AEREO3         <- dist2Line(dataSitios,l3) %>% as.data.frame() %>% select(., distance)
dataSitios$AEREO4         <- dist2Line(dataSitios,l4) %>% as.data.frame() %>% select(., distance)
dataSitios$AEREO5         <- dist2Line(dataSitios,l5) %>% as.data.frame() %>% select(., distance)

dataSitios %<>% as.data.frame()
names(dataSitios) <- c("Mun","lon", "lat", "Ruido", "Antenas", "Dmin", "AEREO1",
                     "AEREO2", "AEREO3", "AEREO4","AEREO5")

dataSitios <- dataSitios %>% as.data.frame() %>%
  mutate(., AEREO = pmin(AEREO1, AEREO2, AEREO3, AEREO4, AEREO5)) %>%
  select(., -AEREO1,-AEREO2, -AEREO3, -AEREO4,-AEREO5)

tab        <- xtable(dataSitios, digits = c(0,0,5,5,1,0,0,0))
print(tab, type = "html")
```
...
The relevant functions to use are: 
1. geosphere:gcIntermediate, which retrieves *n* points in the geodesic from point 1 to point 2. This function returns a list that can be turned into a dataframe.
2. raster:extract(RASTER,SpatialObject), return properties of the raster. In this case we provide raster:SpatialPoints converts Lat,lon point into a SpatialPoint and we feed to extract. Extract finds the cell that contain the point and returns the attribute as a SpatialObject. We convert it to a dataframe.
